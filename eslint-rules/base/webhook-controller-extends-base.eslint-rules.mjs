export default {
  meta: {
    type: 'problem',
    docs: {
      description: 'Webhook controller classes must extend WebhookController, BaseController, or import Webhook/Base controller for custom extensions',
    },
    schema: [],
  },
  create(context) {
    const filename = context.getFilename();
    if (!filename.endsWith('.webhook.controller.ts') && !filename.endsWith('.controller.ts')) return {};
    ...
    return {
      ImportDeclaration(node) {
        ...
        for (const spec of node.specifiers) {
          if (spec.type === 'ImportSpecifier') {
            ...
            if (importedName === 'WebhookController' || importedName === 'BaseController') {
      ...
      ClassDeclaration(node) {
        if (!node.superClass) {
          if (!hasBaseImport) {
            context.report({ node, message: 'Webhook controller classes must extend WebhookController or BaseController, or import the base controllers for custom extensions' });
          }
          return;
        }
        ...
        if (!superName) {
          if (!hasBaseImport) context.report({ node: node.superClass || node, message: 'Webhook controller classes must extend WebhookController or BaseController (unable to resolve superclass)' });
          return;
        }

        const lastPart = superName.includes('.') ? superName.split('.').pop() : superName;
        const allowedGlobals = new Set(['WebhookController', 'BaseController', 'CrudController']);
        ...
        if (!isAllowed && !hasBaseImport) {
          context.report({ node: node.superClass, message: 'Webhook controller classes must extend WebhookController or BaseController, or import the base controllers for custom extensions' });
        }
      },
    };
  },
