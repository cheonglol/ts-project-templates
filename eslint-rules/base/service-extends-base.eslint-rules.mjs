export default {
  meta: {
    type: 'problem',
    docs: {
      description: 'Service classes must extend BaseService, or import BaseService for custom extensions',
    },
    schema: [],
  },
  create(context) {
    ...
    return {
      ImportDeclaration(node) {
        if (typeof node.source.value === 'string' && (node.source.value.includes('base-service') || node.source.value.includes('/class/base') || node.source.value.includes('base/'))) {
          hasBaseImport = true;
        }
        ...
        for (const spec of node.specifiers) {
          if (spec.type === 'ImportSpecifier') {
            ...
            if (importedName === 'BaseService') {
              if (localName) allowedLocalNames.add(localName);
              hasBaseImport = true;
            }
          } else if (spec.type === 'ImportDefaultSpecifier') {
            const localName = spec.local && spec.local.name;
            if (localName && typeof node.source.value === 'string' && (node.source.value.includes('base-service') || node.source.value.includes('/class/base'))) {
            ...
            if (localName) {
              ...
              if (typeof node.source.value === 'string' && (node.source.value.includes('base-service') || node.source.value.includes('/class/base'))) {
                hasBaseImport = true;
              }
            }
          }
      ...
      ClassDeclaration(node) {
        if (!node.superClass) {
          if (!hasBaseImport) {
            context.report({ node, message: 'Service classes must extend BaseService or import BaseService for custom extensions' });
          }
          return;
        }
        ...
        if (!superName) {
          if (!hasBaseImport) context.report({ node: node.superClass || node, message: 'Service classes must extend BaseService (unable to resolve superclass)' });
          return;
        }

        const lastPart = superName.includes('.') ? superName.split('.').pop() : superName;
        const allowedGlobals = new Set(['BaseService']);
        ...
        if (!isAllowed && !hasBaseImport) {
          context.report({ node: node.superClass, message: 'Service classes must extend BaseService or import BaseService for custom extensions' });
        }
      },
    };
  },
